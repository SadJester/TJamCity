...
// 8) Exception safety: constructor throws after N
struct bomb {
	static inline std::atomic<int> live { 0 };
	static inline std::atomic<int> ctors { 0 };
	static inline std::atomic<int> dtors { 0 };
	static inline int throw_after; // set in test
	int tag;
	...
	explicit bomb(int t)
		...
		++live;
		if (n == throw_after) {
			throw_after = INT_MAX; // ensure only once
...
TEST_F(object_pool_fixture, exception_safety_on_construction) {
	bomb::reset();
	ObjectPool<bomb, 8, 4> pool;

	bomb::throw_after = 3;
	// First two succeed, third throws; slot 3 must return to free-list
	auto h1 = pool.acquire(1);
	auto h2 = pool.acquire(2);
	EXPECT_THROW({ auto h3 = pool.acquire(3); (void)h3; }, std::runtime_error);

	// Acquire again should succeed and not leak / double-construct
	auto h4 = pool.acquire(4);
	pool.release(h1);
	pool.release(h2);
	pool.release(h4);

	EXPECT_EQ(bomb::live.load(), 0);
	EXPECT_EQ(bomb::ctors.load(), bomb::dtors.load());
}
