...
namespace {
	...
	static constexpr std::size_t kCapacityLarge = 262144;
...
TEST(SpmcQueueTests, SingleThreadedOrderAndDrain) {
	...
	r.read_all([&](const Q::message_t& m) {
		const auto& p = m.get<MsgKind::A, SeqPayload>();
