...
namespace tjs {
	namespace ui {
		TimeControlWidget::TimeControlWidget(Application& application, QWidget* parent)
			...
			const std::string tooltip = "Number of simulation steps per frame when running";
			stepsLabel->setToolTip(tooltip.data());
			...
			_steps_on_update->setToolTip(tooltip.data());
			...
			// Create time label
			_timeLabel = new QLabel("Time: 13/7/2025/6:0", this);
		...
		void TimeControlWidget::updateTimeLabel() {
			...
			_timeLabel->setText(QString("Time: %1").arg(QString::fromStdString(oss.str())));
		}
