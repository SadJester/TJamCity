...
namespace tjs::ui {
    ...
    void VehicleAnalyzeWidget::initialize() {
        ...
        auto setupAgentCombo = [this, &agents]() {
            ...
            for (const simulation::AgentData& agent : agents) {
    ...
    void VehicleAnalyzeWidget::handleAgentSelection(int index) {
        ...
        auto it = std::find_if(agents.begin(), agents.end(), 
            [agentId](const simulation::AgentData& a) { return a.id == agentId; });
    ...
    void VehicleAnalyzeWidget::updateAgentDetails(const simulation::AgentData* agent) {
        if (!agent) {
            return;
        }
        ...
        switch (agent->behaviour) {
            case simulation::TacticalBehaviour::Normal: behaviourStr = "Normal"; break;
            case simulation::TacticalBehaviour::Aggressive: behaviourStr = "Aggressive"; break;
            case simulation::TacticalBehaviour::Defensive: behaviourStr = "Defensive"; break;
            case simulation::TacticalBehaviour::Emergency: behaviourStr = "Emergency"; break;
            default: behaviourStr = "Unknown";
        }
        _behaviourValue->setText(behaviourStr);

        _currentGoalValue->setText(agent->currentGoal ? 
            QString::number(agent->currentGoal->uid) : "None");
